...
namespace SimplestMeshBaker
{
	public class MeshBaker : EditorWindow
	{
		...
		public static void BakeMeshes(MenuCommand menuCommand)
		{
			...
			var bakedMeshContainer = new GameObject("BakedMeshContainer");
			bakedMeshContainer.transform.SetParent(selectedObject.transform, false);
			...
			if (!separateObjectsWithDifferentMaterials)
			{
				...
				{
					Bake(mesh, vertexes, normals, tangents, colors, uvs, triangles, material, bakedMeshContainer.transform);
				}
				...
				if (vertexes.Count > 0)
				{
					CreateObject(vertexes, normals, tangents, colors, uvs, triangles, material, bakedMeshContainer.transform);
					vertexes.Clear();
					normals.Clear();
					tangents.Clear();
					colors.Clear();
			...
			else
			{
				foreach (var mesh in meshesWithoutMaterials)
				{
					Bake(mesh, vertexes, normals, tangents, colors, uvs, triangles, null, bakedMeshContainer.transform);
				}

				if (vertexes.Count > 0)
				{
					CreateObject(vertexes, normals, tangents, colors, uvs, triangles, null, bakedMeshContainer.transform);
					vertexes.Clear();
					normals.Clear();
					tangents.Clear();
					colors.Clear();
				...
				foreach (var meshToBake in meshesWithMaterials)
				{
					foreach (var mesh in meshToBake.Value)
					{
						Bake(mesh, vertexes, normals, tangents, colors, uvs, triangles, meshToBake.Key, bakedMeshContainer.transform);
					}

					CreateObject(vertexes, normals, tangents, colors, uvs, triangles, meshToBake.Key, bakedMeshContainer.transform);
					vertexes.Clear();
					normals.Clear();
					tangents.Clear();
					colors.Clear();
			...
			EditorUtility.DisplayDialog("Mesh Baker", string.Format("Baked {0} meshes", meshesCount), "Ok");
		}
		...
		private static void CheckMeshAttributes(Mesh mesh, ref bool anyHasNotNormals, ref bool anyHasNotColors, ref bool anyHasNotUVs, ref bool anyHasNormals, ref bool anyHasColors, ref bool anyHasUVs)
		{
		...
		private static void Bake(Mesh mesh, List<Vector3> vertexes, List<Vector3> normals, List<Vector4> tangents, List<Color> colors, List<Vector2> uvs, List<int> triangles, Material material, Transform parent)
		{
			//mesh may not have more than 65000 vertices.
			if (vertexes.Count + mesh.vertexCount > MAX_VERTEX_COUNT_PER_ONE_OBJECT)
			{
				CreateObject(vertexes, normals, tangents, colors, uvs, triangles, material, parent);
				vertexes.Clear();
				normals.Clear();
				tangents.Clear();
				colors.Clear();
		...
		private static void FillOrResolve<T>(T[] source, List<T> distanation, int expectedCount, Resolving resolvingLogic)
		{
		...
		public static void BakeColliders(MenuCommand menuCommand)
		{
			...
			if (colliders.Any())
			{
				var colliderContainer = new GameObject("ColliderContainer");
				colliderContainer.transform.SetParent(selectedObject.transform, false);

				foreach (var collider in colliders)
				{
					var colliderObj = new GameObject(string.Format("{0} Collider", collider.name));
					colliderObj.transform.SetPositionAndRotation(collider.transform.position, collider.transform.rotation);
					colliderObj.transform.SetParent(colliderContainer.transform, true);
					colliderObj.isStatic = collider.gameObject.isStatic;

					// Copy collider to new GameObject
					var clone = colliderObj.AddComponent(collider.GetType()) as Collider;
					collider.CopyTo(clone);
				}
			}
		}
	}
